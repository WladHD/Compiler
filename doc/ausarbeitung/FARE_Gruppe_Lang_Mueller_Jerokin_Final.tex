...
\begin{document}
	...
	Durch erweiterte Annotation der Grammatik wird folgend der AST (Abstract Syntax Tree) durch JJTree generiert (Syntaxanalyse).
...
\newpage
\section{Semantik und Fehlermeldung}
Nach der Erzeugung des AST wird dieser der semantischen Prüfung übergeben.
Hierbei wird gleichzeitig auch die Fehlerbehandlung (und die Codeerzeugung in Kapitel \ref{sec:übersetzung}) abgedeckt.
...
\begin{longtable}{|c|c|}
	...
	19 & WrongArrayDefinitionSemanticException  \\ \midrule
...
\noindent
Folgend werden in Listings \ref{lst:errCode1} bis \ref{lst:errCode18} Beispiele für fehlerhaften Code gezeigt, welche in Exceptions in Tabelle \ref{table:semanticCheck} resultieren.
\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer ExpectedTypeMissmatchSemanticException}, label=lst:errCode1]
...
\newpage
\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer IllegalClassContentSemanticException}, label=lst:errCode2]
for (;;) {} // EXCEPTION -> Außerhalb einer Methode
void main() {}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer IllegalInitializerSemanticException}, label=lst:errCode19]
NoExample f = /* init */;
void main() {
	f.getName() = "dsf"; // EXCEPTION -> Prio 15
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer IllegalOperationSemanticException}, label=lst:errCode3]
void main() {
	int x = true + 1; // EXCEPTION -> boolean + int
	String x1 = "hello" + 1; // KEINE EXCEPTION -> String + _ = String
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer MainMethodWithArgsSemanticException}, label=lst:errCode4]
void main(int x) {} // EXCEPTION -> Main Methode darf keine Args haben
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer MethodDeclaredSemanticException}, label=lst:errCode5]
void main() {}
void main() {} // EXCEPTION -> Methode bereits deklariert
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer MethodNotDeclaredSemanticException}, label=lst:errCode6]
void main() {
	m404(); // EXCEPTION -> Methode noch nicht deklarierts
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer MethodParameterMismatchSemanticException}, label=lst:errCode7]
void main() {
	main(5, true); // EXCEPTION -> Parameter stimmen nicht überein
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer NoMainMethodSemanticException}, label=lst:errCode8]
int x = 5; <EOF> // EXCEPTION -> Keine Main Methode definiert
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer NoReturnSemanticException}, label=lst:errCode9]
int calc() { int x = 5; } // EXCEPTION -> Kein Return Statement
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer NotArrayExceptionSemanticException}, label=lst:errCode10]
void main() {
	int x = 5;
	int y = x[1]; // EXCEPTION -> Arrayzugriff obwohl keine Array
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer ReturnTypeMismatchSemanticException}, label=lst:errCode11]
void main() {}
int calc() {
	return true; // EXCEPTION -> int erwartet, aber boolean bekommen
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer StatementExpectedSemanticException}, label=lst:errCode12]
void main() {
	for(5;;) {} // EXCEPTION -> 5 ist kein Statement
	for(;;6) {} // EXCEPTION -> 6 ist kein Statement
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer UnreachableCodeSemanticException}, label=lst:errCode13]
void main() {
	return;
	int x = 5; // EXCEPTION -> Nicht erreichbar
}
\end{lstlisting}


\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer VariableDeclarationNotAllowedSemanticException}, label=lst:errCode14]
void main() {
	for(;;) int x = 5; // EXCEPTION -> Nicht erlaubt
	while(true) int x = 5; // EXCEPTION -> Nicht erlaubt
	if(true) int x = 5; // EXCEPTION -> Nicht erlaubt
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer VariableDeclaredSemanticException}, label=lst:errCode15]
void main() {
	int x = 5;
	boolean x = false; // EXCEPTION -> Bereits deklariert
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer VariableDeclaredSemanticException}, label=lst:errCode16]
void print(int i) {}
void main() {
	print(x); // EXCEPTION -> x nicht deklariert
}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer VariableNotInitializedSemanticException}, label=lst:errCode17]
	void print(int i) {}
	void main() {
		int x;
		print(x); // EXCEPTION -> x nicht initialisiert
	}
\end{lstlisting}

\begin{lstlisting}[caption={Fehlerhafter Code resultierend in einer WrongArrayDefinitionSemanticException}, label=lst:errCode18]
void main() {
	Set<int> = {1,2,3}; // EXCEPTION -> [] für Set
	int[] = [1,2,3]; // EXCEPTION -> {} für Arrays
	Map<int[], Set<int>> ms = [{1,2,3}: [3,4,5]] // KEINE EXCEPTION
}
\end{lstlisting}
...
\paragraph{Verwendung des Compilers} \mbox{} \\
Das Programm \textit{FARECompiler.jar} akzeptiert zwei Parameter und kann wie folgt aufgerufen werden:

\noindent
\textit{java -jar FARECompiler.jar <InputFile> [OutputFolder]}

\noindent
\textit{InputFile} gibt den Pfad zur Textdatei an, in welcher sich der Quellcode befindet.
\textit{OutputFolder} gibt jeweils den Pfad zum Ordner an, in welchem das kompilierte FARE Programm abgespeichert werden soll.
Wird der \textit{OutputFolder} nicht gesetzt, wird der generierte AST + Java Code nur auf der Konsole angezeigt. Um das angehängte Beispiel auszuführen, nutzt man z. B. folgenden Befehl:

\noindent
\textit{java -jar ../FARECompiler.jar aufgabe.fare ./}
...
Die kompilierte jar kann dann mit folgendem Befehl gestartet werden:

\noindent
\textit{java -jar TranslatorTemplate.jar}
